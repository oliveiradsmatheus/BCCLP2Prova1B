...
export default function DetalhesCandidato(props) {
    ...
    const [listaQuestionamento, setListaQuestionamentos] = useState([]);

    function curtir(candidato) {
        candidato.curtidas += 1;
        props.setListaCandidatos(props.listaCandidatos.filter((item) => {
            return item.id === candidato.id ? candidato : item;
        }))
    }

    function descurtir(candidato) {
        candidato.descurtidas += 1;
        props.setListaCandidatos(props.listaCandidatos.filter((item) => {
            return item.id === candidato.id ? candidato : item;
        }))
    }
    ...
    function manipularMudanca(evento) {
        ...
        setQuestionamento({ ...questionamento, [elemento]: valor });
    ...
    return (
        <Container className="text-center">
            <img alt="foto do candidato" height="300px" src={candidato.avatar} />
            <Card.Body>
                <Card.Title>{"candidato: " + candidato.nome}</Card.Title>
                <Card.Text>
                    <p>{"Email: " + candidato.email}</p>
                    <p>{"Curtidas: " + candidato.curtidas}</p>
                    <p>{"Questionamentos: " + candidato.questionamentos}</p>
                </Card.Text>
                <Button onClick={() => {
                    curtir(candidato)
                    ...
                    {candidato.curtidas}</Button><Button onClick={() => {
                        descurtir(candidato);
                    ...
                    {candidato.descurtidas}</Button>
            </Card.Body>
            <Card>
                {
                    candidato.propostas.map((item, index) => {
                        return (
                            <p>
                                {index + 1} - {item}
                            </p>
            ...
            <Card>
                {
                    listaQuestionamento?.map((item, index) => {
                        return (
                            <p>
                                {index + 1} - {item}
                            </p>
            ...
            <Form.Group>
                <Form.Label>Tem algum questionamento?</Form.Label>
                <Form.Control
                    type="text"
                    id="questionamento"
                    name="questionamento"
                    placeholder="Informe seu questionamento..."
                    value={candidato.novoQuestionamento}
                    onChange={manipularMudanca}
                    required
                />
                <Button onClick={() => {
                    registrarQuestionamento(candidato.novoQuestionamento);
                }} className="mt-1" variant="success">Registrar Questionamento</Button>
            </Form.Group>
